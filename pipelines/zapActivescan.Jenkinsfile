pipeline {
  ...
  stages {
    stage("Test") {
      when {
        expression { params.reportedBy != '' }
      }
      steps {
                sh 'printf "\\e[31mSome tests execution here - user provided his name...\\e[0m\\n"'
                sh 'printf "\\e[31mFOO variables is set to bar\\e[0m\\n"'
      }
    }
    stage('Run ZAP Headless with xvfb') {
      steps {
              ...
              sh 'zap-x.sh -daemon -host 127.0.0.1 -newsession "/tmp/session" -config api.disablekey=true -config scanner.attackOnStart=true -config view.mode=attack -config connection.dnsTtlSuccessfulQueries=-1 -config api.addrs.addr.name=.* -config api.addrs.addr.regex=true &'
      }
    }
    ...
    stage('Run ZAP Active Scan') {
      steps {
        sh """
          set +e
          zap-cli open-url \$SCAN_TARGET
          zap-cli -v spider \$SCAN_TARGET
          zap-cli -v ajax-spider \$SCAN_TARGET
          zap-cli -v active-scan \$SCAN_TARGET
          set -e
        """
      }
    }
        ...
        script {
          if (reportedBy == "") {
            wrap([$class: 'BuildUser']) {
              reportedBy = "${env.BUILD_USER}"
            }
            println "reportedBy:=${reportedBy}"
          }
        sh """
          REPORT_DIR="\$(pwd)/out"
          ...
          zap.sh -export_report -port 8087 \${REPORT_DIR}/\${REPORT_FILE_XHTML} -source_info "\${REPORT_NAME}; \${PREPARED_BY}; CompanyName; \${SCAN_DATE}; \${REPORT_DATE}; Scan \$SCAN_VERSION; Report \$REPORT_VERSION; \${DESCRIPTION}" -alert_severity "t;t;f;t" -alert_details "t;t;t;t;t;t;f;f;f;f" -session "/tmp/cpsession/session" -cmd
        """
